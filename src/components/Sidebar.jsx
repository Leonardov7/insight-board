...
const Sidebar = ({ 
  ...
  const fetchSessions = async () => {
    ...
    if (user) {
      const { data, error } = await supabase
        ...
        .eq('owner_id', user.id) // <--- FILTRO DE PRIVACIDAD
  ...
  const handleToggleStatus = async (e, sessionId, currentStatus) => {
    ...
    const { error } = await supabase.from('sesiones').update({ status: newStatus }).eq('id', sessionId);
  ...
  const handleDeleteSession = async (e, sessionId) => {
    ...
    const { error } = await supabase.from('sesiones').delete().eq('id', sessionId);
    if (!error) {
      if (sessionId === currentSessionId) onNewDebate();
      else fetchSessions();
    }
  };
    ...
    try {
      ...
      if (targets && targets.length > 0) {
        for (const item of targets) {
          ...
          await sendMessage(item.provocation, item.alias, randomColor, item.targetId, currentSessionId, true);
        }
      }
    } catch (error) { console.error("Fallo IA:", error); } finally { setIsAiThinking(false); }
  };
  ...
  return (
    <div className={`fixed top-[70px] bottom-0 right-0 w-80 bg-[#0e0e12]/95 backdrop-blur-2xl border-l border-white/10 z-50 shadow-2xl flex flex-col transition-transform duration-500 ease-in-out font-sans ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      <button onClick={onClose} className="absolute left-[-30px] top-1/2 -translate-y-1/2 bg-[#0e0e12]/95 p-1.5 rounded-l-xl border-l border-y border-white/10 text-slate-500 hover:text-white transition-all shadow-[-5px_0_15px_rgba(0,0,0,0.5)]">
        <ChevronRight size={18} />
      </button>

      <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
        <div className="flex flex-col">
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 flex items-center gap-2"><Database size={14} /> Panel de Control</h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {isAdmin && (
          <section className="border border-white/5 rounded-2xl overflow-hidden bg-white/[0.01]">
            <button onClick={() => setIsHistoryOpen(!isHistoryOpen)} className="w-full p-4 flex items-center justify-between hover:bg-white/[0.03] transition-all">
              <div className="flex items-center gap-3"><History size={14} className={isHistoryOpen ? "text-indigo-400" : "text-slate-500"} /><span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Historial de Sesiones</span></div>
        ...
        {isAdmin && currentSessionId && (
          <section className="p-5 border border-indigo-500/20 rounded-2xl bg-indigo-500/5 space-y-4">
            <h3 className="text-[10px] font-black uppercase text-indigo-400 tracking-[0.2em] flex items-center gap-2"><Zap size={14} fill="currentColor" /> Comentario</h3>
            <button onClick={handleInvokeAI} disabled={isAiThinking} className={`w-full py-4 rounded-xl border transition-all flex items-center justify-center gap-3 font-black uppercase text-[10px] tracking-widest ${isAiThinking ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400 cursor-wait' : 'bg-indigo-600/20 border-indigo-500/40 text-indigo-100 hover:bg-indigo-600 shadow-[0_0_20px_rgba(99,102,241,0.15)]'}`}>
              {isAiThinking ? <Loader2 size={14} className="animate-spin" /> : "Lanzar Comentario"}
            </button>
          </section>
        )}
        ...
        {isAdmin && currentSessionId && (
          <section className="mt-4 p-4 border border-white/5 rounded-2xl bg-white/[0.01]">
            <button onClick={() => setIsAnalyticsOpen(true)} className="w-full p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-all flex flex-col items-center gap-2 group">
              <BarChart3 size={20} className="text-indigo-400 group-hover:scale-110 transition-transform" />
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Abrir Monitor Semántico</span>
            </button>
          </section>
        )}
      </div>
      ...
      {isAdmin && (
        <div className="p-6 border-t border-white/5 bg-white/[0.01] space-y-3">
          <button onClick={onNewDebate} className="w-full py-4 bg-indigo-600/10 hover:bg-indigo-600 hover:text-white text-indigo-400 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 border border-indigo-500/20 transition-all mb-3"><RefreshCw size={14} /> Iniciar Nuevo Debate</button>
          <button onClick={handleSignOut} className="w-full py-4 bg-rose-500/5 hover:bg-rose-500 hover:text-white text-rose-400 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 border border-rose-500/10 transition-all"><LogOut size={14} /> Cerrar Sesión</button>
        </div>
      )}
    </div>
  );
